/**
  ...
  * <tr><td>2020-04-07   	<td> 1.0     <td>Mentos Seetoo  <td>Creator
	* <tr><td>2020-04-17   	<td> 1.1     <td>Sigma Chen			<td>Add collision operation.
  * </table>  
  * 	
  ==============================================================================
                               How to use this Lib
...
*/
bool CoppeliaSim_Client::Start(const char *p_IP, int32_t p_conection_port, int commThreadCycleInMs, bool synchronousMode)
{
	/*Save parameters*/
...
bool CoppeliaSim_Client::Set_startParam(const char *p_IP, int32_t p_conection_port, int commThreadCycleInMs, bool synchronousMode)
{
	if(p_IP == NULL || p_conection_port == 0 || commThreadCycleInMs == 0) return false;
...
_simCollisHandle_Type* CoppeliaSim_Client::Add_Object(std::string full_name)
{
	...
	/*Check the memory*/
	if (p_Collision == NULL) return p_Collision;
...
bool CoppeliaSim_Client::ComWithServer()
{
	...
	if (clientID != -1)
	{
				...
				if (GetIO_Type(OP_Obj.operation) == CLIENT_RO)
				{
					...
					case SIM_NORMAL_IMAGE:
						OP_Obj.read_return_code = simxGetVisionSensorImage(clientID, VisionSensor->obj_handle,
								 VisionSensor->resolution, &VisionSensor->image_buffer, 0, simx_opmode_oneshot);
						break;
					case SIM_DEPTH_IMAGE:
						OP_Obj.read_return_code = simxGetVisionSensorDepthBuffer(clientID, VisionSensor->obj_handle,
								 VisionSensor->resolution, &VisionSensor->depth_buffer, simx_opmode_oneshot);
						break;
					default:
						break;
					}
		...
		for each (auto Collision in Collision_list)
		{
