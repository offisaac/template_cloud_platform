...
///  
///  修订说明：  
///  2019/12/25  MentosSeetoo（SCUT-RobotLab）   为方便扩展接口API，接收部分添加了回调处理函数，
///                                              安全性降低，仅供内部使用，版权归原作者所有。 
//////////////////////////////////////////////////////////////////////////  
...
public:
	...
	*  @see:
	*/
	bool InitPort(UINT  portNo = PORTNUM, UINT  baud = CBR_9600, char  parity = 'N', UINT  databits = 8, UINT  stopsbits = 1, DWORD dwCommEvents = EV_RXCHAR, UINT threadInterval = 5);
	...
	*
	*
	*  @param:  unsigned char data 要发送的数据
	*  @return: bool  发送是否成功
	*  @note:
	*  @see:
	*/
  ...
  *
  *
  *  @param:  unsigned char data 接收到的数据内容
  *  @return: unsigned int  接收到的数据长度
  *  @note:   注册进来的函数将运行在监听线程，收到数据后将会被调用。
  *  @see:
  */
