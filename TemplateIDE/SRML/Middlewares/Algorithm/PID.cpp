...
float myPID::Adjust()
{
  ...
  if(std::abs(Ki) > std::numeric_limits<float>::epsilon()){
    ...
    /*Constrain*/
    integral_e = shit::constrain(integral_e, -I_Term_Max/Ki, I_Term_Max/Ki);
  }
  else{
    integral_e = 0;
  }
  ...
  { 
    ...
    /*Constarin*/
    I_Term = shit::constrain(I_Term, -I_Term_Max, I_Term_Max);
  }
  else{
    /*Clear*/
    I_Term = 0;
  ...
  /* Constarin */
  Out = shit::constrain(Out, -Out_Max, Out_Max);
