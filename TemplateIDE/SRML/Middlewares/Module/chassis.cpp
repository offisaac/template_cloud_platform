...
uint8_t CChassis::speed_control()
{
  ...
  */
  for (uint8_t k = 0; k < WHEEL_NUM; ++k )
     wheel_Out[k] = shit::constrain(wheel_Out[k], (int32_t)-Param.MAX_MOTOR_OUTPUT, (int32_t)Param.MAX_MOTOR_OUTPUT);
...
void CChassis::Set_Target(float target_X, float target_Y ,float target_Z)
{
  ...
  if(Mode == Normal_Speed)
  {
    Command_Velocity.x_speed = shit::constrain(target_X,-1.0f,1.0f)*Co_limit*Param.wheel_max_speed;
    Command_Velocity.y_speed = shit::constrain(target_Y,-1.0f,1.0f)*Co_limit*Param.wheel_max_speed;
    Command_Velocity.z_speed = shit::constrain(target_Z,-1.0f,1.0f)*Param.coefficient_z*Param.wheel_max_speed;
  }
...
{
  Param.coefficient_slow   = shit::constrain(slow, 0.0f, 1.0f);
  Param.coefficient_normal = shit::constrain(normal, 0.0f, 1.0f);
  Param.coefficient_fast   = shit::constrain(fast, 0.0f, 1.0f);
  Param.coefficient_z      = shit::constrain(z, 0.0f, 1.0f);
}

void CChassis::Set_AccelerationParam(int16_t launch, int16_t normal, uint16_t brake)
{
	launch = shit::constrain((int16_t)launch, (int16_t)-32765, (int16_t)32765);
	normal = shit::constrain((int16_t)normal, (int16_t)-32765, (int16_t)32765);
	brake = shit::constrain((int16_t)normal, (int16_t)0, (int16_t)65535);
