/**
  ...
  *  2021-03-21		Lingzi    5.0.0		接收数据包格式适应21年规则 	
...
void referee_Classdef::RefereeHandle(uint8_t *data_buf)
{
    switch(((FrameHeader *)data_buf)->CmdID) {											//取出解包结果的cmd_id
			...
			case RobotComData_ID:																				//若为数据交互的数据帧，则交给其他成员函数进行处理							
...
void referee_Classdef::RobotInteractiveHandle(robot_interactive_data_t* RobotInteractiveData_t)
{
	if(GameRobotState.robot_id == RobotInteractiveData_t->receiver_ID && GameRobotState.robot_id != 0) {		//如果确实是发往本机器人的交互数据帧，则接受该帧
		if(RobotInteractiveData_t->data_cmd_id == RobotComData_ID) {											//根据发送者的ID号归纳数据段并存储到对应区域
			if(RobotInteractiveData_t->sender_ID > 0x100)														//蓝方数据接收
				memcpy(&robot_rec_data[RobotInteractiveData_t->sender_ID - 0x100], RobotInteractiveData_t->data, RobotInteractiveData_t->data[0]);
			else																								//红方数据接收
				memcpy(&robot_rec_data[RobotInteractiveData_t->sender_ID], RobotInteractiveData_t->data, RobotInteractiveData_t->data[0]);
		}
	}
}
...
void referee_Classdef::Calc_Robot_ID(uint8_t local_id)
{
	if(local_id !=0 )																	//referee connection successful
	{
		if(local_id < 10)	
		{
			...
			robot_client_ID.client = 0x100 + local_id;									//计算当前机器人的ID号（红方）
		}
		else	
		{
			...
			robot_client_ID.local = local_id;	
			robot_client_ID.client = 0x0164 + (local_id - 100);							//计算当前机器人的ID号（蓝方）
		}
	}
}
...
/**
  ...
  * @note  	Referee.CV_ToOtherRobot(Referee.robot_client_ID.hero, 123, 1);
...
/**
 * @brief 打包机器人之间的交互数据并发送（问题：官方约定是整包最大不能超过128字节，但这里似乎只对数据段进行了操作）
 * @param 
 */
...
void referee_Classdef::send_toReferee(uint16_t _cmd_id, uint8_t* _data, uint16_t _data_len, receiverType_e _receiver)
{
	...
	memcpy((void*)temp, &send_frame_header, header_len);																	//将帧头装入缓存区
	memcpy((void*)(temp+header_len), _data, _data_len);																		//将数据段转入缓存区	
	
	* (__packed short *)(&temp[header_len + _data_len]) = Get_CRC16_Check_Sum(temp,header_len + _data_len,0xffff);			//获取整帧的CRC16校验码，并直接填入缓存区
	...
	while(send_cnt != 0)
	{
		...
		{
			HAL_UART_Transmit_DMA(refereeUart,temp,total_len);																//延时已到，则发送
			next_send_time = now_time + float(total_len) / 3720 * 1000;														//计算下一次允许传输的时间，2020赛季官方约定传输速率为3720bps
 ...
 * @note Referee.line_drawing(ADD_PICTURE, 400,500,1000,900,GREEN, test);
 * @param _operate_type:官方协议手册上的绘图部分有定义
 * @retval 
 */
void referee_Classdef::line_drawing(drawOperate_e _operate_type, uint16_t startx,uint16_t starty,uint16_t endx,uint16_t endy,colorType_e vcolor, uint8_t name[])
{
	...
	drawing.end_x=endx;
	drawing.end_y=endy;
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制一条直线
}
...
void referee_Classdef::rectangle_drawing(drawOperate_e _operate_type, uint16_t startx,uint16_t starty,uint16_t length_,uint16_t width_,colorType_e vcolor, uint8_t name[])
{
	...
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制一个矩形
}
...
void referee_Classdef::circle_drawing(drawOperate_e _operate_type, uint16_t centrex,uint16_t centrey,uint16_t r,colorType_e vcolor, uint8_t name[])
{
	...
	drawing.radius=r;
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制一个圆
}
/**
 * @brief 
...
void referee_Classdef::oval_drawing(drawOperate_e _operate_type, uint16_t centrex,uint16_t centrey,uint16_t minor_semi_axis,uint16_t major_semi_axis,colorType_e vcolor, uint8_t name[])
{
	...
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制一个椭圆
...
void referee_Classdef::arc_drawing(drawOperate_e _operate_type, uint16_t centrex,uint16_t centrey,uint16_t endx,uint16_t endy,int16_t start_angle_,int16_t end_angle_,colorType_e vcolor, uint8_t name[])
{
	...
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制一段圆弧
...
void referee_Classdef::float_drawing(drawOperate_e _operate_type, uint16_t startx,uint16_t starty, colorType_e vcolor, float data, uint8_t name[])
{
	...
	drawing.start_angle = 10;																						//这里是设置字体大小，需要调试
	...
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制浮点数
...
void referee_Classdef::int_drawing(drawOperate_e _operate_type, uint16_t startx,uint16_t starty,uint16_t size, uint8_t length,uint8_t character[], colorType_e vcolor, int32_t data,uint8_t name[])
{
	...
	memcpy((void*)drawing.radius, &data, 4);																		//将32位整数赋值到drawing结构体中

	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)&drawing, sizeof(drawing));					//客户端绘制整数
...
{
	char_length = shit::constrain(char_length, (uint8_t)0, (uint8_t)30);											//将字符串长度约束在30个之内
...
/**
 * @brief draw_ruler，只绘制一条标尺，在循环中调用。返回1时退出循环体
...
{	
	...
	static uint32_t last_sys_time = 0;																				//记录上一次绘制的时间
	...
	if(draw_cnt<5) /* 画5次 */
	{
		if(_sys_time - 200 >= last_sys_time)																		//单位: ms，频率不能大于10Hz
		{
			...
			if(scale_cnt == 0)
			{
				line_drawing(ADD_PICTURE, start_x,start_y,start_x,start_y-(sacle_num+1)*step,_color, name); 		//标尺竖线				
			}
			else
			{
				if(scale_cnt%2 == 1)
					line_drawing(ADD_PICTURE, start_x,axis_temp,start_x+scale_long,axis_temp,_color, name);   		//标尺竖线上的长横线
				else
					line_drawing(ADD_PICTURE, start_x,axis_temp,start_x+scale_short,axis_temp,_color, name);  		//标尺竖线上的短横线
			}
...
{
	static int draw_cnt = 0;																						//辅助命名组成图案的每一条直线
	static uint8_t name[3] = "xx";
	uint16_t point_cnt = length / 2;																				//记录需要绘制的点数
	
	for(uint16_t i = 0; i < point_cnt; i+=2)																		//每两点构成一条直线，故需要i+=2，以上一根线的终点为下一根线的起点
	{		
		name[0] = draw_cnt / 255;																					//直线起名，0x
		name[1] = draw_cnt % 255;		
		draw_cnt++;
